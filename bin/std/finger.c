...
status finger(string str) {
  ...
  if(str && sscanf(str,"%s %s",str,tmp) != 2) {
      ...
      if(!restore_object(SAVE_PLAYER+str)) {
        if(!restore_object(SAVE_FIRST+str)) {
          if(!restore_object(SAVE_NO_BANISH+str)) {
            notify_fail ("No player with that name exists.\n");
            return 0;
          }
        }
      }
    ...
    if(ob) {
      if ((security_level <= this_player()->query_security_level() && is_invis)
      || !is_invis) {
  ...
  finger += ((extra_info == "") ? "No Plan." : "Plan:\n"+ extra_info) +"\n";
    if(!this_player()->ansi_on()) finger = this_player()->filter_ansi(finger);
#ifdef PAGER
    pager_ob = clone_object(PAGER);
    move_object(pager_ob, this_player());
    pager_ob->page(finger);
#else
    write(finger);
#endif /* PAGER */
    return 1;
