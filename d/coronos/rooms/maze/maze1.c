...
status e0() {
    if (exit_num == 0)
        call_other(this_player(), "move_player", "north#"+PATH+"maze2");
    else
        call_other(this_player(), "move_player", "north#"+PATH+"well");
    return 1;
}

status e1() {
    if (exit_num == 1)
	call_other(this_player(), "move_player", "south#"+PATH+"maze2");
    else
	call_other(this_player(), "move_player", "south#"+PATH+"well");
    return 1;
}

status e2() {
    if (exit_num == 2)
	call_other(this_player(), "move_player", "east#"+PATH+"maze2");
    else
...
status e3() {
    if (exit_num == 3)
	call_other(this_player(), "move_player", "west#"+PATH+"maze2");
    else
