...
void long(status wiz) {
  ::long(wiz);
  write(query_name() +" offers healing to those who pray.\n");
}
...
void welcome(object obj) {
    ...
    write("Aurora says: Welcome, "+ this_player()->query_name() +".\n");

    if(this_player()->query_level() < 2) {
	write("\tI see you are new in these lands. If you need some help,\n"+
		"\tgo outside the temple, onto the main road, and find Bastian.\n"+
		"\tHe'll help you out.\n");
    }
    else {
...
status pray(string str) {
  ...
  if(!this_player()->query_ghost()) {
    ...
    if(this_player()->query_level() < 2) {
      write("\nAurora says: Remember, if you are in need of help, find \n"+
            "             Bastian the guide. He has maps and helpful \n"+
	    "             advice for new travellers to the city Ille Coronos.\n");
    }
...
status neutralize_poison() {
  ...
  if(poison) {
     tell_object(target,"You feel the poison leave your body.\n");
      write("Aurora neutralizes the poison in "+ target->query_name() 
           +"'s system.\n"); 
      say("Aurora neutralizes the poison in "+
          target->query_name()+".\n",target);
    destruct(poison);
    target->set_constitution((int)target->query_constitution());
  }
  else {
      write("Aurora says: You are not poisoned.\n");  
  }
  this_player()->add_money(-mon);
  return 1;
}
...
status restoration() {
  ...
  if(!target->query_level_drained()) {
      write("Aurora says: You have not level drained.\n");
  }
  else {
    tell_object(target,"You have been restored a level.\n");
    say("Aurora has restored a level to "+
        target->query_name() +".\n");
