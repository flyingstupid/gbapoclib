...
void reset(status arg) { 
  ...
  if(!a2) {
    ...
    a2 -> set_plural("suits of leather armour");
...
status go() {
  if((int)this_player()->query_level() < 5) {
    tell_room(environment(), "Orc bars the way.\n");
    return 1;
  }
  if((string)this_player()->query_race() == "dark elf") {
    tell_room(environment(), "Orc says: Hello master, please pass.\n");
    return 0;
  }
  else {
    tell_room(environment(), "Orc exclaims: Hey, you can't go in there!\n");
    return 1;
  }
  return 0;
}
