...
void reset (status arg){
  ...
  if(!present("drow warrior")) {
    move_object(clone_object(
    "/d/coronos/w/newstar/newbie/npc/drow1"),this_object());
  }
 
  if(!present("drow scout")) {
    move_object(clone_object(
    "/d/coronos/w/newstar/newbie/npc/drow2"),this_object());
  }
  ...
  set_long(
    "The somber atmosphere comes to a peak in this corner of the dark forest.\n"+
    "The thorny underbrush seems especially dense, leaving virtually no room\n"+
    "for movement.  The sinister canopy of the ancient trees shadows the\n"+
    "forest floor below with its large, dark leaves.  An eerie mist snakes\n"+
    "along the thin path that veers off to the west and south.");
  set_day_desc(
    "  The shadows\n"+
  ...
  set_items(({
    ...
    "path",
      ...
      "passage.  The path continues off to the south and to the west.\n",
    "root#roots",
  ...
  set_weather(1, 4, 0);
  set_exits(({
      "/d/coronos/w/newstar/newbie/rooms/forest3", "south",
      "/d/coronos/w/newstar/newbie/rooms/forest1", "west",
      }));
}
