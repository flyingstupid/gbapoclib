...
void reset(status arg) {
  ...
  reset_doors(arg);
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room17",
    "direction", "west door",
  ...
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room18",
    "direction", "east door",
    "description", "A plain wooden door .\n",
    "closed",
    "door number", 2,
 ...
 "long narrow hallway#narrow hallway#long hallway#hallway#dormitory",
  ...
  set_exits(({
     "d/coronos/w/nightbringer/college/room13",  "south",
     "d/coronos/w/nightbringer/college/room19", "north",
...
status read(string str) {
  if(str == "plaque on west door") {
    write(RED_F+"-+= 110 [MICHELLE NEILSON/BRIDGETTE PHIEFER] =+-\n"+OFF);

    return 1;
  
  }
  if(str == "plaque on east door") {
    write(RED_F+"-+= 109 [HEAVEN HAWKINS/NINA RASON] =+-\n"+OFF);
