...
void reset(status arg) {
  ...
  reset_doors(arg);
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room27",
    "direction", "west door",
  ...
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room28",
    "direction", "east door",
    "description", "A plain wooden door .\n",
    "closed",
    "door number", 2,
 ...
 "long narrow hallway#narrow hallway#long hallway#hallway#dormitory",
  ...
  set_exits(({
     "d/coronos/w/nightbringer/college/room23",  "south",
     "d/coronos/w/nightbringer/college/room29", "north",
...
status read(string str) {
  if(str == "plaque on west door") {
    write(RED_F+"-+= 202 [OTIK KALOV/STEEL TRUEBLADE] =+-\n"+OFF);

    return 1;
  
  }
  if(str == "plaque on east door") {
    write(RED_F+"-+= 201 [GAVIN LIGHTFOOT/WILLIAM WALLACE] =+-\n"+OFF);
