...
void reset(status arg) {
  ...
  reset_doors(arg);
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room33",
    "direction", "west door",
  ...
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room34",
    "direction", "east door",
    "description", "A plain wooden door .\n",
    "closed",
    "door number", 2,
  ...
  set_long(
     ...
     "grey srone. There is a plaque on the east and west doors.\n");
 ...
 "long narrow hallway#narrow hallway#long hallway#hallway#dormitory",
  ...
  set_exits(({
     "d/coronos/w/nightbringer/college/room29",  "south",
     "d/coronos/w/nightbringer/college/room35", "north",
...
status read(string str) {
  if(str == "plaque on west door") {
    write(RED_F+"-+= 202 [GEORGE BRAVEHEART/THOR BARLOW] =+-\n"+OFF);

    return 1;
  
  }
  if(str == "plaque on east door") {
    write(RED_F+"-+= 201 [PHILLIPE FRENCH/KEVIN STONEY] =+-\n"+OFF);
