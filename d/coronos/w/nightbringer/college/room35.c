...
void reset(status arg) {
  ...
  reset_doors(arg);
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room36",
    "direction", "west door",
  ...
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room37",
    "direction", "east door",
    "description", "A plain wooden door .\n",
    "closed",
    "door number", 2,
 ...
 "long narrow hallway#narrow hallway#long hallway#hallway#dormitory",
  ...
  set_exits(({
     "d/coronos/w/nightbringer/college/room32",  "south",
     "d/coronos/w/nightbringer/college/room38",  "north",
       }));
   set_weather(1,2,0);
}
...
status read(string str) {
  if(str == "plaque on west door") {
    write(RED_F+"-+= 208 [JACK DANIELS/JOHN EARHARDT] =+-\n"+OFF);

    return 1;
  
  }
  if(str == "plaque on east door") {
    write(RED_F+"-=+ 207 [NORDA PROI/NIL SPAAR] =+-\n"+OFF);
