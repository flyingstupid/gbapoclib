...
void reset(status arg) {
  ...
  reset_doors(arg);
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room8",
    "direction", "west door",
  ...
  load_door(({
    "destination", "d/coronos/w/nightbringer/college/room9",
    "direction", "east door",
    "description", "A plain wooden door .\n",
    "closed",
    "door number", 2,
 ...
 "long narrow hallway#narrow hallway#long hallway#hallway#dormitory",
  ...
  set_exits(({
     "d/coronos/w/nightbringer/college/room4",  "south",
     "d/coronos/w/nightbringer/college/room10", "north",
...
status read(string str) {
  if(str == "plaque on west door") {
    write(RED_F+"-+= 104 [TONI BRIGHTBLADE/ALICIA CROWNGUARD] =+-\n"+OFF);

    return 1;
  
  }
  if(str == "plaque on east door") {
    write(RED_F+"-+= 103 [TIKA D'GHOE/LAURANNA SYKES] =+-\n"+OFF);
