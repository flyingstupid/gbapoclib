...
void reset(status arg) {
  ...
  set_weather(2,1,0);
  set_long(
	"The library of the Towers of High Sorcery is a huge one indeed.\n"+
	"Tall book shelves tower ever upward toward a domed ceiling,    \n"+
	"inlaid with gold and silver. There are several chairs and tables\n"+
	"with candle holders for students to come and do their research \n"+
	"in magic at all hours of the day and night. The atmosphere here\n"+
	"is quiet and peaceful.\n");
  set_short("the tower of sorcery");
  set_exits(({
	"d/sorcery/w/angel/study2", "north",
	"d/sorcery/w/angel/study1", "south",
  }));
  set_sign_message(
     "\t-=[ The Library of the Council of Mages ]=- \n"+
...
mixed *write_catalogue(string str, int silently) {
  ...
  if(!silently) {
    write("\t\tThe Books of the Tower of High Sorcery Library\n");
