...
#include <i_to_str.h>
#include <process.h>       /* process_msg() fn */
...
mapping query_all_info() {
  ...
  switch(tmp) {
    case "Armour" :       case "Weapon" :       case "Container" :
    case "Drink" :        case "Food"   :
    case "Magic Armour" : case "Magic Weapon" : case "Wand" :
...
varargs status show_inventory(object ob) {
  ...
  for(i = 0; i < size; i++) 
  {
      ...
      if(sht) 
      {
          ...
          sh = explode(sht," ");
          sh = filter(sh,"filter_nul_str",this_object());
          if(sizeof(sh)) 
          {
              sht = implode(sh," ");      
