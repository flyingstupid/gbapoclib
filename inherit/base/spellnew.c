      ...
      if(this_object()->query_npc()) {
        ...
        if(!sizeof(COMPONENT)) {
          ...
          switch(SPELL_CLASS) {
            ...
            case "psionic" : write("mental focus\n"); break;
  ...
  if(!objectp(SPELL_TYPE)) {
    if(SPELL_NAME) {
      switch(SPELL_CLASS) {
        ...
        case "bard" :
          write("You begin to sing up a storm . . . \n");
          say(NAME +" starts to sing up a storm . . .\n");
        break;
...
#define ROBES_OF_ALIGNMENT
      ...
      if(TARGET) {
        ...
        if(ob) TARGET = ob;
        if(PASSIVE && !TARGET) TARGET = this_player();
        if(!living(ob)) { /* cast at non-living object */ 
          ...
          if(!ob->cast_spell(this_player(),TARGET,0,spell_dam)) {
            ...
            if(ROOM_MSG)   
              say((string)this_object()->process_msg(ROOM_MSG), this_object());
          }
          break;
        }
      }
...
int spell_hit(int dmg,     /* spell damage */
  ...
  if(!passive) {
    ...
    if(!thief_bs)
      save = (int)this_object()->query_wisdom() + 1 +
           (int)this_object()->query_save_bonus();
    else
      save = (int)this_object()->query_intelligence() + 1 + 
             (int)this_object()->query_save_bonus();
    if(save < 2) save = 2; 
    if(immune_type && query_spell_immunity("-"+immune_type)) {
      save /= 2;
  ...
  if(!prev && !this_object()->add_secondary_attacker(this_player())) {
    if(!thief_bs) {
  ...
  if(room_msg) say((string)this_object()->process_msg(room_msg), this_object());
  if(target_msg)
