...
string query_exit_filename_all( string str )
{
    mixed ret = query_exit_filename(str);
...
int query_door_exit_status(string dir)
{
    int i;
...
int valid_exit( string str )
{
    int i = 0;
    mixed *all_exits = query_exits();
    for( i = 0; i < sizeof( all_exits ); i+=2 )
    {
        if( str == all_exits[i+1] )
            return 1;
    }
...
void clean_up() {
  ...
  if(!dest_dir || !sizeof(dest_dir)) {
    ...
    tell_room(this_object(),
	"Gully dwarf leaves "+dest_dir[(random(sizeof(dest_dir)/2)*2)+1]+"\n");
  }
  this_object()->reset(0);
  return;
}
