...
status withdraw(string str) {
  ...
  if(sscanf(str, "%d %s coin%s", amount, tmp1, tmp2) ||
     sscanf(str, "%d %s", amount, tmp1)) {
    if(tmp1 == "platinum") amount *= PLATINUM;
    else if(tmp1 == "gold")        amount *= GOLD;
...
/* exchange rates for coins can be found in /obj/money.c
