/* DEBUG object, If any of the interactive player objects fail, this
   can act as a temporary interactive object for debugging problems
 */
...
static status wiz(string str) { 
  ...
  ob = filter(users(),"filter_wizards",this_object());
...
status who() { 
  ...
  who = filter(who,"filter_wizards",this_object());
  num_wiz = sizeof(who);
  if(i == 1) {
...
static void logon2(string str) { 
  ...
  str = lower_case(str);  
  name = str;
  restore_object(WIZARD_SAVE_DIR+name);
  if(!secure(SEC5)) {  
    write("\n\nSorry. 'Sands of Time' has had a break down!!\n");
    remove_call_out("time_out"); 
    destruct(this_object());  
    return;
  }
...
static void logon3() { 
  ...
  write("You are using default debug object, and have been given access\n"+
        "to fix the problem. Admin is putting a trust in your hands. If\n"+
        "don't feel confident that you can fix the problem, then leave it\n"+
        "until an Admin Creator Arrives.\n\n"+
        ...
        "greater to log in. Debug has only a limited number of commands,\n"+
