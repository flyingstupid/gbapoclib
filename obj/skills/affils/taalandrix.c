...
int query_extra_spell_dmg(int damage, string school) {
  ...
  if(FILE == "taalandrix" && tmp != "neutral") return 0;


  if(!stringp(school)) return 1;
/** taalandrix clerics dont get these things *
  switch(school) {
...
void reset(status arg) {
  ...
  set_long(
	...
	"custom. The medalion is circular, with a kitten's paw in the \n"+
	"centre; the symbol of mischief in the multiverse.\n");
