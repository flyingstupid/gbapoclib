...
void time_out() {
  if(scryer) {
    tell_object(scryer,"The remote viewer powers down.\n");
  }
...
level) {
  ...
  if((ob = present("remoteviewer", caster))) {
    if(ob->query_scry()) {
  ...
  if(!(ob = find_living(who)) || ob->query_security_level()) {
    write("You cannot remote view "+ capitalize(who) +".\n");
    destruct(this_object());
  }
  else if(ob->query_invis() || present("No Scry", ob)) {
    write("Something prevents you from remote viewing "+ capitalize(who)
+".\n");
    destruct(this_object());
  }
  else {
    scry_controller = clone_object("obj/skills/remoteviewer");
