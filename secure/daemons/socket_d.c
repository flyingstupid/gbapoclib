...
//
// Heaven7 Mudlib
// Original work from Wunderland Mudlib
// (Many thanks to Fiona for donating this 
// code to the community)
//
// secure/daemon/socketd.c  --  internet-sockets (needs xerq)
//
...
// Access for internal output cachenetstat.
...
static void debug(varargs mixed* x) {
  ...
  foreach (y : x) {
    i = strstr(txt, "%O");
    if (i >= 0) txt[i..i+1] = sprintf("%O",y);
    else txt += " " + sprintf("%O",y);
  }
