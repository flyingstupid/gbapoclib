...
/*
 * Note: this is a work in progress and currently doesn't work
 * (hence it's not included in the simul's yet)
 * We know parse_command() is deprecated and I want to provide a 
 * simul for it when it finally does go away.
 * -Phaedrus
 */
...
mixed *parse_cmd(string cmd, mixed obarr, string pattern);
private void store_value(int pos, mixed what);
static void store_words_slice(int pos, mixed *warr, int from, int to);
static int sub_parse(mixed *obarr, mixed *patarr, int  pix_in, mixed *warr, int  cix_in);
private int one_parse(mixed *obarr, string pat, mixed *warr, int   cix_in);
private int number_parse(mixed *obarr, mixed *warr, int   cix_in);
private int item_parse(mixed *obarr, mixed *warr, int   cix_in);
private int single_parse(mixed *obarr, mixed *warr, int   cix_in);
private int prepos_parse(mixed *warr, int   cix_in, mixed prepos);
private int match_object(int obix, mixed *warr, int   cix_in, int   plur);
static int find_string(string str, mixed *warr, int   cix_in);
private int check_adjectiv(int obix, mixed *warr, int from, int to);
...
mixed *parse_cmd(string cmd, mixed obarr, string pattern) {
    ...
    if (!pointerp(obarr)) error("Bad argument 2 to parse_command().\n");
	...
	if (parse_patarr[pix] == "%s") {
		...
		do {
		     ...
		     * pattern
		     */
		    fail = sub_parse(obarr, parse_patarr, & pix,
				      parse_warr, & cix);
		    if (fail) {
			cix = ++ocix;
			pix = fpix;
...
private static int
sub_parse(mixed *obarr, mixed *patarr, int   pix_in, 
	  mixed *warr, int   cix_in)
{
    int cix, pix;
    int fail;
    ...
    fail = one_parse(obarr, patarr[pix], warr, &cix);
...
private int
one_parse(mixed *obarr, string pat, mixed *warr, int cix_in)
{
    int ch, fail;
    string str1, str2;
...
private int
number_parse(mixed *obarr, mixed *warr, int cix_in) {
    ...
    /* This next double loop is incredibly stupid. */
...
private int
item_parse(mixed *obarr, mixed *warr, int cix_in) {
    mixed *tmp, *ret;
...
private int
living_parse(mixed *obarr, array warr, int cix_in, int fail)
{
    mixed *live;
    object ob;
    int obix, tix;
...
private int
single_parse(mixed *obarr, mixed *warr, int cix_in)
{
...
private int
prepos_parse(mixed *warr, int cix_in, mixed prepos) {
    mixed *tarr;
    string tmp;
    int pix, tix;

    if (!prepos || !pointerp(prepos))
	prepos = prepos_list;
...
private int
match_object(int obix, mixed *warr, int   cix_in, int   plur) {
    mixed *ids;
...
static int
find_string(string str, mixed *warr, int   cix_in)
{
    int fpos;
    string p1;
    mixed *split;
...
check_adjectiv(int obix, mixed *warr, int from, int to)
{
    ...
    if (pointerp(adjid_list[obix]))
	ids = adjid_list[obix];
    else
	ids = 0;

    for (fail = 0, il = from; il <= to; il++) {
	if ((member(ids, warr[il]) < 0) &&
	    (member(adjid_list_d, warr[il] ) < 0))
	    fail = 1;
    }
	...
	for (back = to; back >= il; back--) {	/* back from adjN to adj[il] */
	    ...
	    if ((member(ids, adstr) < 0) &&
		(member(adjid_list_d, adstr) < 0))
		continue;
	    else {
