//
//  wiz/kate/open/medic/frenzy.c - increase combat - requires 1 serum, 1 syringe and 1 enzyme, Increases combat - enhancing + (enhancing * random (1-2)) duration is based on (preventing * random (5-10)) + 60
//  Second Chance
//
#include <mudlib.h>
...
status frenzy(string target){
    ...
    victim->adj_combat(enhanceAmount);
    tell_object(this_player(), "You inject a neurostimulant into " + victim->query_name() + " causing them to enter into a state of increased combat ability ("+ enhanceAmount + ")\n");
    tell_object(victim, this_player()->query_name() + " injects you with a strange fluid.\n" +
                "Your vision becomes more clear and your mind focuses on the combat at hand.\n");
