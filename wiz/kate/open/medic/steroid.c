//
//  wiz/kate/open/medic/steroid.c - increase combat - requires 1 serum, 1 syringe and 1 enzyme, Increases Dex base + enhancing + (enhancing * random (1-2)) duration is based on (preventing * random (5-10)) + 60
//  Second Chance
//
...
status steroid(string target){
    ...
    victim->set_combat(enhanceAmount);
    tell_object(this_player(), "You swiftly inject " + victim->query_name() + " with a steroid serum causing them to become stronger ("+ enhanceAmount + ")\n");
    tell_object(victim, this_player()->query_name() + " injects you with a strange serum.\n" +
                "You feel your muscles bulge and your heart rate increase.\n");
